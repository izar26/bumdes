                ...
                <form action="{{ route('admin.manajemen-data.unit_usaha.store') }}" method="POST">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="jenis_usaha" class="form-control @error('jenis_usaha') is-invalid @enderror" id="jenis_usaha" placeholder="Contoh: Perdagangan, Jasa" value="{{ old('jenis_usaha') }}" required>
                            @error('jenis_usaha')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="penanggung_jawab_ids[]" id="penanggung_jawab_ids" class="form-control select2 @error('penanggung_jawab_ids') is-invalid @enderror" multiple="multiple" style="width: 100%;">
                                @foreach($users as $user)
                                    <option value="{{ $user->user_id }}">{{ $user->name }} ({{ $user->getRoleNames()->first() }})</option>
                                @endforeach
                            </select>
